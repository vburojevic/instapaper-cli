...
func TestPrintPlainOutputs(t *testing.T) {
	...
	if got, want := b.String(), "1\t1\t0.5000\tExample\thttps://example.com\n"; got != want {
		t.Fatalf("plain bookmarks mismatch: %q", got)
	}
	...
	if got, want := f.String(), "2\t3\tWork\n"; got != want {
		t.Fatalf("plain folders mismatch: %q", got)
	}
	...
	if got, want := h.String(), "4\t1\t10\tQuote\n"; got != want {
		t.Fatalf("plain highlights mismatch: %q", got)
	}
}
